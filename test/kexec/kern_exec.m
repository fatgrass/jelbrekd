...
static mach_port_t prepare_user_client()
{
    ...
    if (err != KERN_SUCCESS) {
        fprintf(stderr,"unable to get user client connection");
        exit(EXIT_FAILURE);
    }
    
    fprintf(stderr, "got user client: 0x%x", user_client);
    return user_client;
}
...
bool init_kexecute()
{
    ...
    WriteKernel64(fake_vtable + 8 * 0xB7, get_ret());
